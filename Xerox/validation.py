...
def get_predicted_labels_and_median_time():
    output_df = pd.read_csv('output_last.csv')
...
for ne in [210]:
    print  '\n\n\n\nNo.  of estimators =' , ne
    for m_samples_split in range(3,4):
        print '\n\n\nmin_samples_split =' ,m_samples_split
        for m_samples_leaf in range(3,4):
            print '\nmin_samples_leaf =' , m_samples_leaf
            clf = AdaBoostClassifier(base_estimator = DecisionTreeClassifier(min_samples_split=m_samples_split,  min_samples_leaf =m_samples_leaf) , n_estimators=ne) 
            ...
            for i in range(len(predicted_probabilities)):
                if predicted_probabilities[i][1] > 0.9995:
            ...
            wr  = csv.writer(open('output_last.csv' , 'w+'))
