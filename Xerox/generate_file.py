...
#
#score_evaluation_df = pd.read_csv('id_time_features.csv')
#X_val = score_evaluation_df.drop(['ID','TIME'],axis = 1)
## """
#clf = AdaBoostClassifier(base_estimator = DecisionTreeClassifier(min_samples_leaf =7) , n_estimators=75)   
#clf.fit(X_train , Y_train)
#predicted_probabilities = clf.predict_proba(X_val.values)
#predicted_labels = clf.predict(X_val).tolist()
# predicted_labels = []
# for i in range(len(predicted_probabilities)):
#     if predicted_probabilities[i][1] > 0.6:
#         predicted_labels.append(1)
#     else :
#         predicted_labels.append(0)
# temp = score_evaluation_df[['ID' , 'TIME']]
# temp['PREDICTION'] = pd.Series(predicted_labels)
# temp['PROBABILITY'] = pd.Series(predicted_probabilities[:,1])
# output_df = temp
...
w  =  csv.writer(open('id_time_features.csv','wb'))
w.writerow(['ID','TIME']+selected_features)

min_id_val = df_age_val['ID'].min()
max_id_val = df_age_val['ID'].max()
 
grouped_by_id_val = df_variables_combined_val.groupby('ID')
#print 'Reached the most time-taking part\n'
for i in range(min_id_val , max_id_val + 1):
    ...
    for j,row in current_patient_df_val.iterrows():
        ...
        #input feature vector for our classifier model
            ...
            feature_vector_val = get_patient_feature_values_val(i, data_so_far_df_val)
 
            feature_vector_val.append(l15_first_in_icu_val)
            feature_vector_val.append(l22_first_in_icu_val)
           
            w.writerow( [i , row[1] ] + feature_vector_val )
        counter = counter + 1
